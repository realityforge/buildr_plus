...
BuildrPlus::FeatureManager.feature(:deps => [:libs]) do |f|
  f.enhance(:Config) do
    ...
    def server_generators
      ...
      generators << [:ee_messages, :ee_messages_qa, :ee_exceptions, :ejb_service_facades, :ee_filter, :ejb_test_qa, :ejb_test_service_test] if BuildrPlus::FeatureManager.activated?(:ejb)
    ...
    def server_compile_deps
      ...
      dependencies << Buildr.artifacts(BuildrPlus::Libs.simple_keycloak_service) if BuildrPlus::FeatureManager.activated?(:keycloak) && BuildrPlus::FeatureManager.activated?(:replicant)
