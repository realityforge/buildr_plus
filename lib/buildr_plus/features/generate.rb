...
BuildrPlus::FeatureManager.feature(:generate) do |f|
  ...
  f.enhance(:ProjectExtension) do
    ...
    def all_resgen_keep_file_patterns
      BuildrPlus::Generate.keep_file_patterns + self.keep_file_patterns
    end
    ...
    def resgen_keep_file_names
      if File.exist?(resgen_keep_filename_registry)
        base = File.expand_path(project._('src'))
        IO.read(resgen_keep_filename_registry).split("\n").collect { |f| "#{base}/#{f}" }
      else
        []
      end
    end
    ...
    def resgen_keep_filename_registry
      ...
      t = project.task 'collect_keep_files' do
        if project.inline_generated_source?
          ...
          if project.name.to_s.end_with?(':user-experience')
            ...
            files = []
            Dir["#{target_dir}/**/*"].each do |file_name|
              if !File.directory?(file_name) && !IO.read(file_name).include?('DO NOT EDIT: File is auto-generated by Resgen')
                files << ::Buildr::Util.relative_path(file_name, target_dir)
              end
            end
            files += project.extra_keep_file_names
            ...
            existing = IO.read("#{target_dir}/resgen_keep_files.txt") rescue ''
            if existing != new_content
              FileUtils.mkdir_p target_dir
